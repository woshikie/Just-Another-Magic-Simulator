
using System.Collections;

namespace Card
{
	public class PadeemConsulofInnovation : Card
	{
		/*
		Artifacts you control have hexproof.
At the beginning of your upkeep, if you control the artifact with the highest converted mana cost or tied for the highest converted mana cost, draw a card.
		*/
		private const string UniqueCardName = "Padeem, Consul of Innovation";
		public PadeemConsulofInnovation() : base(UniqueCardName) {
				
		}
		protected override void OnConstructed()
		{
			Types = new string[] {  };
			Colors = new string[] {  };
			SubTypes = new string[] { "vedalken", "vedalken" };
			SuperTypes = new string[] {  };
		}

	}
}
